...
import { DosageForm, DosageFormService } from '../../services/dosage-form.service';
...
@Component({
  ...
  template: `
    <div class="page-wrap animate-fadein-up">
      ...
      <div id="main-sticky-zone" class="sticky-header">
        <div class="page-head pt-2 pb-1 px-4">
          <div style="margin-left: 12px;">
            <h1 class="page-title">Dosage Form Master</h1>
            <p class="page-sub text-xs">Medicine forms like Tablet, Syrup, Injection.</p>
          </div>
        ...
        <div class="table-toolbar px-4 py-2">
          ...
          <div class="flex items-center gap-4">
            <span class="result-count font-semibold">{{ filteredDosageForms().length }} results</span>
            <div class="summary-row">
              <div class="chip chip-teal"><i class="pi pi-box"></i> {{ dosageForms().length }} Total</div>
      ...
      <div class="px-4 pb-4">
        <div class="table-card">
          ...
          <div class="table-responsive">
            <p-table [value]="filteredDosageForms()" 
              ...
              <ng-template pTemplate="body" let-d>
                <tr>
                  <td><span class="badge badge-slate-light font-mono">{{ d.code }}</span></td>
                  <td><span class="med-name text-sm">{{ d.name }}</span></td>
                  <td class="text-muted text-xs">{{ d.description || '—' }}</td>
                  <td>
                    <button class="status-toggle-btn"
                      ...
                      <span class="toggle-label">{{ d.isActive ? 'Active' : 'Inactive' }}</span>
                    </button>
                  </td>
                  <td alignFrozen="right" pFrozenColumn>
                    <div class="action-btns">
                      <button class="act-btn act-edit" title="Edit" (click)="openEdit(d)"><i class="pi pi-pencil"></i></button>
                      <button class="act-btn act-del" title="Delete" (click)="confirmDelete(d)"><i class="pi pi-trash"></i></button>
                    </div>
                  </td>
                </tr>
              </ng-template>
              <ng-template pTemplate="emptymessage">
                <tr><td colspan="5"><div class="empty-state"><i class="pi pi-box empty-icon"></i><p class="empty-text">No dosage forms found</p></div></td></tr>
              </ng-template>
            </p-table>
          </div>
        </div>
      ...
      <p-dialog [header]="editMode ? 'Edit Dosage Form' : 'Add New Dosage Form'" [(visible)]="showDialog" [modal]="true" [style]="{width:'420px'}" [closable]="true" styleClass="premium-dialog">
        <form [formGroup]="form" class="dialog-form">
          <div class="form-row">
            <div class="form-group">
              <label>Code (Auto-Generated)</label>
              <input pInputText formControlName="code" [readonly]="true" class="field-readonly" placeholder="DSG-AUTO"/>
            </div>
            <div class="form-group">
              <label>Name *</label>
              <input pInputText formControlName="name" placeholder="e.g. Tablet" [class.ng-invalid]="isInvalid('name')"/>
...
export class DosageFormListComponent implements OnInit {
  dosageForms = signal<DosageForm[]>([]);
  searchText = ''; showDialog = false; editMode = false;
  form: FormGroup;
  private editId: number | null = null;

  filteredDosageForms = computed(() => {
    const q = this.searchText.toLowerCase().trim();
    if (!q) return this.dosageForms();
    return this.dosageForms().filter(d => d.name?.toLowerCase().includes(q) || d.code?.toLowerCase().includes(q));
  });
  activeCount   = computed(() => this.dosageForms().filter(d => d.isActive).length);
  inactiveCount = computed(() => this.dosageForms().filter(d => !d.isActive).length);

  constructor(private dosageFormService: DosageFormService, private fb: FormBuilder, private confirmationService: ConfirmationService, private messageService: MessageService) {
  ...
  ngOnInit() { this.load(); }
  load() { this.dosageFormService.getAll().subscribe({ next: d => this.dosageForms.set(d), error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load.' }) }); }
  openAdd() { 
    this.editMode = false; this.editId = null; this.form.reset({ isActive: true }); 
    this.dosageFormService.getNextCode('DSG').subscribe(res => { this.form.patchValue({ code: res.code }); });
    this.showDialog = true; 
  }
  openEdit(d: DosageForm) { this.editMode = true; this.editId = d.dosageFormId; this.form.patchValue(d); this.showDialog = true; }
  closeDialog() { this.showDialog = false; this.form.reset({ isActive: true }); }
  save() {
    if (this.form.invalid) return;
    const val = { ...this.form.value, dosageFormId: this.editId ?? 0 };
    const obs = this.editMode && this.editId ? this.dosageFormService.update(this.editId, val) : this.dosageFormService.create(val);
    obs.subscribe({
      next: () => { 
        this.messageService.add({ severity: 'success', summary: 'Success', detail: `Dosage Form ${this.editMode ? 'updated' : 'added'}.` }); 
  ...
  confirmDelete(d: DosageForm) {
    this.confirmationService.confirm({
      message: `Delete dosage form "<b>${d.name}</b>"?`, header: 'Confirm Delete', icon: 'pi pi-exclamation-triangle',
      accept: () => this.dosageFormService.delete(d.dosageFormId).subscribe({ next: () => { this.messageService.add({ severity: 'success', summary: 'Deleted', detail: 'Dosage Form deleted.' }); this.load(); }, error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Delete failed.' }) })
    });
  }
