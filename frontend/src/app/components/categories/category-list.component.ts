...
@Component({
  ...
  template: `
    <div class="page-wrap animate-fadein-up" *ngIf="isSystemAdmin() || hasPermission('Master Data')">
      ...
      <div id="main-sticky-zone" class="sticky-header">
        ...
        <div class="table-toolbar px-4 py-2">
          ...
          <div class="flex items-center gap-4">
            <span class="result-count font-semibold">{{ filteredCategories().length }} results</span>
            <div class="summary-row">
              <div class="chip chip-teal"><i class="pi pi-tags"></i> {{ categories().length }} Total</div>
          ...
          <div class="table-responsive">
            <p-table [value]="filteredCategories()" 
              ...
              <ng-template pTemplate="body" let-c>
                <tr>
                  <td><span class="badge badge-slate-light font-mono">{{ c.code }}</span></td>
                  <td><span class="med-name text-sm">{{ c.name }}</span></td>
              ...
              <ng-template pTemplate="emptymessage">
                <tr><td colspan="5"><div class="empty-state"><i class="pi pi-tags empty-icon"></i><p class="empty-text">No categories found</p></div></td></tr>
              </ng-template>
            </p-table>
          </div>
        </div>
      ...
      <p-dialog [header]="editMode ? 'Edit Category' : 'Add New Category'" [(visible)]="showDialog" [modal]="true" [style]="{width:'420px'}" [closable]="true" styleClass="premium-dialog">
