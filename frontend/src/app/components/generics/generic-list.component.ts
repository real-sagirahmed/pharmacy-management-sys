...
import { Generic, GenericService } from '../../services/generic.service';
...
@Component({
  ...
  template: `
    ...
    <div class="page-wrap animate-fadein-up">
      <div class="page-head">
        <div><h1 class="page-title">Generic (Salt) Master</h1><p class="page-sub">Manage generic salt names like Paracetamol, Amoxicillin.</p></div>
        <button class="btn-primary" (click)="openAdd()"><i class="pi pi-plus"></i> Add Generic</button>
      </div>
      <div class="summary-row">
        <div class="chip chip-teal"><i class="pi pi-list"></i><span>{{ generics().length }} Total</span></div>
      ...
      <div class="table-card">
        <div class="table-toolbar">
          ...
          <span class="result-count">{{ filteredGenerics().length }} results</span>
        </div>
        <p-table [value]="filteredGenerics()" [paginator]="true" [rows]="10" [responsiveLayout]="'scroll'" styleClass="p-datatable-sm" [rowHover]="true">
          ...
          <ng-template pTemplate="body" let-g>
            <tr>
              <td><span class="badge badge-slate">{{ g.code }}</span></td>
              <td class="font-semibold">{{ g.name }}</td>
              <td class="text-muted">{{ g.description || '—' }}</td>
              <td>
                <p-toggleswitch [(ngModel)]="g.isActive" (onChange)="toggleStatus(g)"></p-toggleswitch>
              </td>
              <td>
                <div class="action-btns">
                  <button class="act-btn act-edit" title="Edit" (click)="openEdit(g)"><i class="pi pi-pencil"></i></button>
                  <button class="act-btn act-del" title="Delete" (click)="confirmDelete(g)"><i class="pi pi-trash"></i></button>
                </div>
              </td>
            </tr>
          </ng-template>
          <ng-template pTemplate="emptymessage">
            <tr><td colspan="5"><div class="empty-state"><i class="pi pi-inbox empty-icon"></i><p class="empty-text">No generics found</p></div></td></tr>
          </ng-template>
        </p-table>
      </div>
      <p-dialog [header]="editMode ? 'Edit Generic' : 'Add New Generic'" [(visible)]="showDialog" [modal]="true" [style]="{width:'420px'}" [closable]="true">
        <form [formGroup]="form" class="dialog-form">
          <div class="form-row">
            ...
            <div class="form-group">
              <label>Name *</label>
              <input pInputText formControlName="name" placeholder="e.g. Paracetamol" [class.ng-invalid]="isInvalid('name')"/>
...
export class GenericListComponent implements OnInit {
  generics = signal<Generic[]>([]);
  searchText = ''; showDialog = false; editMode = false;
  form: FormGroup;
  private editId: number | null = null;

  filteredGenerics = computed(() => {
    const q = this.searchText.toLowerCase().trim();
    if (!q) return this.generics();
    return this.generics().filter(g => g.name?.toLowerCase().includes(q) || g.code?.toLowerCase().includes(q));
  });
  activeCount   = computed(() => this.generics().filter(g => g.isActive).length);
  inactiveCount = computed(() => this.generics().filter(g => !g.isActive).length);

  constructor(private genericService: GenericService, private fb: FormBuilder, private confirmationService: ConfirmationService, private messageService: MessageService) {
  ...
  ngOnInit() { this.load(); }
  load() { this.genericService.getAll().subscribe({ next: d => this.generics.set(d), error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load.' }) }); }
  openAdd() { 
    this.editMode = false; 
    this.editId = null; 
    this.form.reset({ isActive: true }); 
    this.genericService.getNextCode('GEN').subscribe(res => {
      this.form.patchValue({ code: res.code });
    });
    this.showDialog = true; 
  }
  openEdit(g: Generic) { this.editMode = true; this.editId = g.genericId; this.form.patchValue(g); this.showDialog = true; }
  closeDialog() { this.showDialog = false; this.form.reset({ isActive: true }); }
  save() {
    if (this.form.invalid) return;
    const val = { ...this.form.value, genericId: this.editId ?? 0 };
    const obs = this.editMode && this.editId ? this.genericService.update(this.editId, val) : this.genericService.create(val);
    obs.subscribe({
      next: () => { 
        this.messageService.add({ severity: 'success', summary: 'Success', detail: `Generic ${this.editMode ? 'updated' : 'added'}.` }); 
        this.closeDialog(); 
        this.load(); 
      },
      error: (e) => {
  ...
  confirmDelete(g: Generic) {
    this.confirmationService.confirm({
      message: `Delete generic "<b>${g.name}</b>"?`, header: 'Confirm Delete', icon: 'pi pi-exclamation-triangle',
      accept: () => this.genericService.delete(g.genericId).subscribe({ next: () => { this.messageService.add({ severity: 'success', summary: 'Deleted', detail: 'Generic deleted.' }); this.load(); }, error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Delete failed.' }) })
    });
  }
