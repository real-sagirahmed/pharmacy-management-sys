...
import { CommonStrength, CommonStrengthService } from '../../services/common-strength.service';
...
@Component({
  selector: 'app-common-strength-list',
  standalone: true,
  ...
  template: `
    <div class="page-wrap animate-fadein-up">
      ...
      <div id="main-sticky-zone" class="sticky-header">
        <div class="page-head pt-2 pb-1 px-4">
          <div style="margin-left: 12px;">
            <h1 class="page-title">Strength Master</h1>
            <p class="page-sub text-xs">Common medicine strengths like 500mg, 20mg, 5ml.</p>
          </div>
        ...
        <div class="table-toolbar px-4 py-2">
          ...
          <div class="flex items-center gap-4">
            <span class="result-count font-semibold">{{ filteredStrengths().length }} results</span>
            <div class="summary-row">
              <div class="chip chip-teal"><i class="pi pi-bolt"></i> {{ commonStrengths().length }} Total</div>
      ...
      <div class="px-4 pb-4">
        <div class="table-card">
          ...
          <div class="table-responsive">
            <p-table [value]="filteredStrengths()" 
              ...
              <ng-template pTemplate="body" let-s>
                <tr>
                  <td><span class="badge badge-slate-light font-mono">{{ s.code }}</span></td>
                  <td><span class="med-name text-sm">{{ s.name }}</span></td>
                  <td class="text-muted text-xs">{{ s.description || '—' }}</td>
                  <td>
                  ...
                  <td alignFrozen="right" pFrozenColumn>
                    <div class="action-btns">
                      <button class="act-btn act-edit" title="Edit" (click)="openEdit(s)"><i class="pi pi-pencil"></i></button>
                      <button class="act-btn act-del" title="Delete" (click)="confirmDelete(s)"><i class="pi pi-trash"></i></button>
                    </div>
                  </td>
                </tr>
              </ng-template>
              <ng-template pTemplate="emptymessage">
                <tr><td colspan="5"><div class="empty-state"><i class="pi pi-bolt empty-icon"></i><p class="empty-text">No strengths found</p></div></td></tr>
              </ng-template>
            </p-table>
          </div>
        </div>
      ...
      <p-dialog [header]="editMode ? 'Edit Strength' : 'Add New Strength'" [(visible)]="showDialog" [modal]="true" [style]="{width:'420px'}" [closable]="true" styleClass="premium-dialog">
        <form [formGroup]="form" class="dialog-form">
          <div class="form-row">
            <div class="form-group">
              <label>Code (Auto-Generated)</label>
              <input pInputText formControlName="code" [readonly]="true" class="field-readonly" placeholder="STR-AUTO"/>
            </div>
            <div class="form-group">
              <label>Name *</label>
              <input pInputText formControlName="name" placeholder="e.g. 500mg" [class.ng-invalid]="isInvalid('name')"/>
  ...
  styles: [`
    ...
    ::ng-deep .p-datatable .p-datatable-thead > tr > th {
      background-color: #f1f5f9 !important; color: #0d9488 !important; font-weight: 800 !important; font-size: 0.75rem !important; text-transform: uppercase !important; letter-spacing: 0.7px !important; padding: 10px 12px !important; border-bottom: 2.5px solid #0d9488 !important;
    }
    ...
    .form-group label { font-size: .8rem; font-weight: 700; color: #1e293b; }
...
export class CommonStrengthListComponent implements OnInit {
  commonStrengths = signal<CommonStrength[]>([]);
  searchText = ''; showDialog = false; editMode = false;
  form: FormGroup;
  private editId: number | null = null;

  filteredStrengths = computed(() => {
    const q = this.searchText.toLowerCase().trim();
    if (!q) return this.commonStrengths();
    return this.commonStrengths().filter(s => s.name?.toLowerCase().includes(q) || s.code?.toLowerCase().includes(q));
  });
  activeCount   = computed(() => this.commonStrengths().filter(s => s.isActive).length);
  inactiveCount = computed(() => this.commonStrengths().filter(s => !s.isActive).length);

  constructor(private strengthService: CommonStrengthService, private fb: FormBuilder, private confirmationService: ConfirmationService, private messageService: MessageService) {
    this.form = this.fb.group({ code: ['', Validators.required], name: ['', [Validators.required, Validators.maxLength(100)]], description: [''], isActive: [true] });
  }
  ngOnInit() { this.load(); }
  load() { this.strengthService.getAll().subscribe({ next: d => this.commonStrengths.set(d), error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load.' }) }); }
  openAdd() { 
    this.editMode = false; this.editId = null; this.form.reset({ isActive: true }); 
    this.strengthService.getNextCode('STR').subscribe(res => { this.form.patchValue({ code: res.code }); });
    this.showDialog = true; 
  }
  openEdit(s: CommonStrength) { this.editMode = true; this.editId = s.commonStrengthId; this.form.patchValue(s); this.showDialog = true; }
  closeDialog() { this.showDialog = false; this.form.reset({ isActive: true }); }
  save() {
    if (this.form.invalid) return;
    const val = { ...this.form.value, commonStrengthId: this.editId ?? 0 };
    const obs = this.editMode && this.editId ? this.strengthService.update(this.editId, val) : this.strengthService.create(val);
    obs.subscribe({
      next: () => { 
        this.messageService.add({ severity: 'success', summary: 'Success', detail: `Strength ${this.editMode ? 'updated' : 'added'}.` }); 
  ...
  confirmDelete(s: CommonStrength) {
    this.confirmationService.confirm({
      message: `Delete strength "<b>${s.name}</b>"?`, header: 'Confirm Delete', icon: 'pi pi-exclamation-triangle',
      accept: () => this.strengthService.delete(s.commonStrengthId).subscribe({ next: () => { this.messageService.add({ severity: 'success', summary: 'Deleted', detail: 'Strength deleted.' }); this.load(); }, error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Delete failed.' }) })
    });
  }
