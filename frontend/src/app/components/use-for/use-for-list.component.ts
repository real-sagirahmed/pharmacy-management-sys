...
import { UseFor, UseForService } from '../../services/use-for.service';
...
@Component({
  ...
  template: `
    <div class="page-wrap animate-fadein-up">
      ...
      <div id="main-sticky-zone" class="sticky-header">
        <div class="page-head pt-2 pb-1 px-4">
          <div style="margin-left: 12px;">
            <h1 class="page-title">Use For Master</h1>
            <p class="page-sub text-xs">Indications like Fever, Pain, Infection, Cough.</p>
          </div>
        ...
        <div class="table-toolbar px-4 py-2">
          ...
          <div class="flex items-center gap-4">
            <span class="result-count font-semibold">{{ filteredUseFors().length }} results</span>
            <div class="summary-row">
              <div class="chip chip-teal"><i class="pi pi-heart"></i> {{ useFors().length }} Total</div>
            ...
            <p-table [value]="filteredUseFors()" 
              ...
              <ng-template pTemplate="header">
                <tr>
                  ...
                  <th pSortableColumn="name" style="min-width: 180px;">Name <p-sortIcon field="name"></p-sortIcon></th>
              ...
              <ng-template pTemplate="body" let-u>
                <tr>
                  ...
                  <td class="text-muted text-xs">{{ u.remarks || '—' }}</td>
                  <td>
              ...
              <ng-template pTemplate="emptymessage">
                <tr><td colspan="5"><div class="empty-state"><i class="pi pi-heart empty-icon"></i><p class="empty-text">No indications found</p></div></td></tr>
              </ng-template>
            </p-table>
          </div>
        </div>
      ...
      <!-- Add / Edit Dialog -->
      <p-dialog [header]="editMode ? 'Edit Indication' : 'Add New Indication'" [(visible)]="showDialog"
                [modal]="true" [style]="{width:'420px'}" [closable]="true" styleClass="premium-dialog">
        <form [formGroup]="form" class="dialog-form">
          <div class="form-row">
            <div class="form-group">
              <label>Code (Auto-Generated)</label>
              <input pInputText formControlName="code" [readonly]="true" class="field-readonly" placeholder="USF-AUTO"/>
            </div>
            <div class="form-group">
              <label>Name *</label>
              <input pInputText formControlName="name" placeholder="e.g. Fever" [class.ng-invalid]="isInvalid('name')"/>
...
export class UseForListComponent implements OnInit {
  useFors = signal<UseFor[]>([]);
  searchText = ''; showDialog = false; editMode = false;
  form: FormGroup;
  private editId: number | null = null;

  filteredUseFors = computed(() => {
    const q = this.searchText.toLowerCase().trim();
    if (!q) return this.useFors();
    return this.useFors().filter(u => u.name?.toLowerCase().includes(q) || u.code?.toLowerCase().includes(q));
  });
  activeCount   = computed(() => this.useFors().filter(u => u.isActive).length);
  inactiveCount = computed(() => this.useFors().filter(u => !u.isActive).length);

  constructor(private useForService: UseForService, private fb: FormBuilder, private confirmationService: ConfirmationService, private messageService: MessageService) {
    this.form = this.fb.group({ code: ['', Validators.required], name: ['', [Validators.required, Validators.maxLength(100)]], remarks: [''], isActive: [true] });
  }
  ngOnInit() { this.load(); }
  load() { this.useForService.getAll().subscribe({ next: d => this.useFors.set(d), error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load.' }) }); }
  openAdd() { 
    this.editMode = false; this.editId = null; this.form.reset({ isActive: true }); 
    this.useForService.getNextCode('USF').subscribe(res => { this.form.patchValue({ code: res.code }); });
    this.showDialog = true; 
  }
  openEdit(u: UseFor) { this.editMode = true; this.editId = u.useForId; this.form.patchValue(u); this.showDialog = true; }
  closeDialog() { this.showDialog = false; this.form.reset({ isActive: true }); }
  save() {
    if (this.form.invalid) return;
    const val = { ...this.form.value, useForId: this.editId ?? 0 };
    const obs = this.editMode && this.editId ? this.useForService.update(this.editId, val) : this.useForService.create(val);
    obs.subscribe({
      next: () => { 
        this.messageService.add({ severity: 'success', summary: 'Success', detail: `Indication ${this.editMode ? 'updated' : 'added'}.` }); 
  ...
  confirmDelete(u: UseFor) {
    this.confirmationService.confirm({
      message: `Delete indication "<b>${u.name}</b>"?`, header: 'Confirm Delete', icon: 'pi pi-exclamation-triangle',
      accept: () => this.useForService.delete(u.useForId).subscribe({ next: () => { this.messageService.add({ severity: 'success', summary: 'Deleted', detail: 'Indication deleted.' }); this.load(); }, error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Delete failed.' }) })
    });
  }
