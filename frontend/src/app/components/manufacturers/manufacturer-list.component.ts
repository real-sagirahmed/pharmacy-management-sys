...
import { Manufacturer, ManufacturerService } from '../../services/manufacturer.service';
...
@Component({
  ...
  template: `
    <div class="page-wrap animate-fadein-up">
      ...
      <div id="main-sticky-zone" class="sticky-header">
        <div class="page-head pt-2 pb-1 px-4">
          <div style="margin-left: 12px;">
            <h1 class="page-title">Manufacturer Master</h1>
            <p class="page-sub text-xs">Medicine manufacturers & pharmaceutical companies.</p>
          </div>
        ...
        <div class="table-toolbar px-4 py-2">
          ...
          <div class="flex items-center gap-4">
            <span class="result-count font-semibold">{{ filteredManufacturers().length }} results</span>
            <div class="summary-row">
              <div class="chip chip-teal"><i class="pi pi-briefcase"></i> {{ manufacturers().length }} Total</div>
      ...
      <div class="px-4 pb-4">
        <div class="table-card">
          ...
          <div class="table-responsive">
            <p-table [value]="filteredManufacturers()" 
              ...
              <ng-template pTemplate="body" let-m>
                <tr>
                  <td><span class="badge badge-slate-light font-mono">{{ m.code }}</span></td>
                  <td><span class="med-name text-sm">{{ m.name }}</span></td>
                  <td><span class="text-muted text-xs">{{ m.description || '—' }}</span></td>
                  <td>
                    <button class="status-toggle-btn"
                      ...
                      <span class="toggle-label">{{ m.isActive ? 'Active' : 'Inactive' }}</span>
                    </button>
                  </td>
                  <td alignFrozen="right" pFrozenColumn>
                    <div class="action-btns">
                      <button class="act-btn act-edit" title="Edit" (click)="openEdit(m)"><i class="pi pi-pencil"></i></button>
                      <button class="act-btn act-del" title="Delete" (click)="confirmDelete(m)"><i class="pi pi-trash"></i></button>
                    </div>
                  </td>
                </tr>
              </ng-template>
              <ng-template pTemplate="emptymessage">
                <tr><td colspan="5"><div class="empty-state"><i class="pi pi-inbox empty-icon"></i><p class="empty-text">No manufacturers found</p></div></td></tr>
              </ng-template>
            </p-table>
          </div>
        </div>
      ...
      <p-dialog [header]="editMode ? 'Edit Manufacturer' : 'Add New Manufacturer'" [(visible)]="showDialog" [modal]="true" [style]="{width:'420px'}" [closable]="true" styleClass="premium-dialog">
        <form [formGroup]="form" class="dialog-form">
          <div class="form-row">
            <div class="form-group">
              <label>Code (Auto-Generated)</label>
              <input pInputText formControlName="code" [readonly]="true" class="field-readonly" placeholder="MFG-AUTO"/>
            </div>
            <div class="form-group">
              <label>Name *</label>
              <input pInputText formControlName="name" placeholder="e.g. Square Pharma" [class.ng-invalid]="isInvalid('name')"/>
  ...
  styles: [`
    ...
    .chip-teal { background: #ccfbf1; color: #0f766e; } .chip-green { background: #dcfce7; color: #15803d; } .chip-slate { background: #f1f5f9; color: #1e293b; }
...
export class ManufacturerListComponent implements OnInit {
  ...
  filteredManufacturers = computed(() => {
    ...
    return this.manufacturers().filter(m => m.name?.toLowerCase().includes(q) || m.code?.toLowerCase().includes(q));
  });
  activeCount   = computed(() => this.manufacturers().filter(m => m.isActive).length);
  inactiveCount = computed(() => this.manufacturers().filter(m => !m.isActive).length);

  constructor(private manufacturerService: ManufacturerService, private fb: FormBuilder, private confirmationService: ConfirmationService, private messageService: MessageService) {
  ...
  ngOnInit() { this.load(); }
  load() { this.manufacturerService.getAll().subscribe({ next: d => this.manufacturers.set(d), error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load.' }) }); }
  openAdd() { 
    this.editMode = false; this.editId = null; this.form.reset({ isActive: true }); 
    this.manufacturerService.getNextCode('MFG').subscribe(res => { this.form.patchValue({ code: res.code }); });
    this.showDialog = true; 
  }
  openEdit(m: Manufacturer) { this.editMode = true; this.editId = m.manufacturerId; this.form.patchValue(m); this.showDialog = true; }
  closeDialog() { this.showDialog = false; this.form.reset({ isActive: true }); }
  save() {
    if (this.form.invalid) return;
    const val = { ...this.form.value, manufacturerId: this.editId ?? 0 };
    const obs = this.editMode && this.editId ? this.manufacturerService.update(this.editId, val) : this.manufacturerService.create(val);
    obs.subscribe({
      next: () => { 
        this.messageService.add({ severity: 'success', summary: 'Success', detail: `Manufacturer ${this.editMode ? 'updated' : 'added'}.` }); 
  ...
  confirmDelete(m: Manufacturer) {
    this.confirmationService.confirm({
      message: `Delete manufacturer "<b>${m.name}</b>"?`, header: 'Confirm Delete', icon: 'pi pi-exclamation-triangle',
      accept: () => this.manufacturerService.delete(m.manufacturerId).subscribe({ next: () => { this.messageService.add({ severity: 'success', summary: 'Deleted', detail: 'Manufacturer deleted.' }); this.load(); }, error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Delete failed.' }) })
    });
  }
